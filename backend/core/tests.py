...
class ModelCreationTest(TestCase):
    ...
    def test_create_product(
        self,
    ):  # Renomeei de 'test_ciar_produto_view' para seguir o padrão de teste de modelo
        """
    ...
    def test_create_oferta(self):
        ...
        self.assertEqual(
            oferta.preco, decimal.Decimal("8500.50")
        )  # Compare como Decimal
        self.assertIsNotNone(oferta.data_captura)
        self.assertTrue(isinstance(oferta, Oferta))
        expected_str = (
            "Oferta de Notebook Apple MacBook Air na Amazon Brasil por R$8500.50"
        )
    ...
    def test_oferta_unique_together_different_time(self):
        ...
        Oferta.objects.create(
            produto=produto,
            loja=loja,
            preco=decimal.Decimal("290.00"),
            data_captura=future_time,
        )
...
# Exemplo de teste para verificar o menor preço (usando funções de utils.py se existirem) # noqa: E501
...
class ProductInfoUtilsTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.produto_com_ofertas = Produto.objects.create(
            nome="Produto Teste com Ofertas",
            categoria=cls.categoria,
            adicionado_por=cls.admin_user,
        )
        # Usamos .create sem data_captura aqui, pois o model tem auto_now_add=True
        # Para garantir que haja diferença no tempo para os testes, criaremos em segundos diferentes # noqa: E501
        cls.oferta1 = Oferta.objects.create(
            produto=cls.produto_com_ofertas,
            loja=cls.loja1,
            preco=decimal.Decimal("100.00"),
        )
        ...
        cls.oferta2 = Oferta.objects.create(
            produto=cls.produto_com_ofertas,
            loja=cls.loja2,
            preco=decimal.Decimal("90.00"),
        )
        timezone.now() + timedelta(seconds=1)
        cls.oferta3 = Oferta.objects.create(
            produto=cls.produto_com_ofertas,
            loja=cls.loja1,
            preco=decimal.Decimal("110.00"),
        )  # Preço maior

        # Produto sem ofertas
        cls.produto_sem_ofertas = Produto.objects.create(
            nome="Produto Teste sem Ofertas",
            categoria=cls.categoria,
            adicionado_por=cls.admin_user,
        )
    ...
    def test_get_product_info_lowest_price(self):
        ...
        from .utils import (
            get_product_info,
        )  # Importa aqui para evitar circular imports se utils.py depende de models

        product_info = get_product_info(self.produto_com_ofertas.id)
        self.assertIsNotNone(product_info)
        self.assertEqual(
            product_info["min_price"], 90.00
        )  # Espera o menor preço das ofertas
    ...
    def test_get_product_info_no_offers(self):
        ...
        )  # Espera None se não houver ofertas
    ...
    def test_search_products(self):
        ...
        Produto.objects.create(
            ...
            adicionado_por=self.admin_user,
            descricao="Ótima para fotos",
        )
        Produto.objects.create(
            nome="Lente 50mm", categoria=self.categoria, adicionado_por=self.admin_user
        )
        ...
        self.assertEqual(
            len(all_results), Produto.objects.count()
        )  # Deve retornar todos os produtos existentes

        # Teste de menor preço na busca
        # offer_for_camera = Oferta.objects.create(
        #    produto=Produto.objects.get(nome="Câmera DSLR"),
        #    loja=self.loja1,
        #    preco=decimal.Decimal("1500.00"),
        # )

        results_with_price = search_products("câmera")
        camera_result = next(
            (p for p in results_with_price if p["name"] == "Câmera DSLR"), None
        )
        self.assertIsNotNone(camera_result)
        self.assertEqual(
            camera_result["min_price"], 1500.00
        )  # Converte para float para comparação


class CreateProductViewTest(
    TestCase
):  # Renomeei de 'ViewTest' para ser mais específico
    def setUp(self):
        """
    ...
    def test_create_product_view_get_authenticated(self):
        ...
        )  # Use 'create_product' conforme URL
        self.assertEqual(response.status_code, 200)
        self.assertContains(
            response, "Adicionar Novo Produto"
    ...
    def test_create_product_view_get_not_staff(self):
        ...
        self.assertEqual(
            response.status_code, 302
        )  # Deve redirecionar para a página de login ou para '/'
        self.assertIn("/login/?next=", response.url)
    ...
    def test_create_product_view_post_success(self):
        ...
        self.assertRedirects(
            response, reverse("product_catalog")
        )  # Redireciona para o catálogo
        self.assertEqual(
            Produto.objects.count(), product_count_before + 1
        )  # Um novo produto deve ter sido criado
        ...
        self.assertEqual(
            str(messages[0]), "Produto 'Livro de Teste' adicionado com sucesso!"
        )
    ...
    def test_create_product_view_post_invalid(self):
        ...
        )  # Não redireciona, re-renderiza o formulário
        self.assertEqual(
            Produto.objects.count(), product_count_before
        )  # Nenhum produto deve ser criado

        # Verifica se as mensagens de erro estão presentes no HTML de resposta
        self.assertContains(
            response, "Erro no campo &#x27;nome&#x27;: Este campo é obrigatório."
        )  # HTML escaped
...
class ManageOffersViewTest(TestCase):
    def setUp(self):
        ...
        self.product1 = Produto.objects.create(
            nome="Celular XYZ", adicionado_por=self.admin_user
        )
        self.product2 = Produto.objects.create(
            nome="Fones Bluetooth", adicionado_por=self.admin_user
        )
    ...
    def test_manage_offers_view_get_unauthenticated(self):
        """
        Testa se a view de gerenciamento de ofertas redireciona usuários não autenticados # noqa: E501.
        """
        response = self.client.get(reverse("manage_offers"))
    ...
    def test_manage_offers_add_new_offer_success(self):
        ...
        self.assertEqual(
            str(messages[0]),
            f"Oferta para '{self.product1.nome}' na '{self.store1.nome}' adicionada com"
            f" adicionada com sucesso por R$199.99!",
        )
    ...
    def test_manage_offers_update_existing_offer_success(self):
        ...
        updated_offer = Oferta.objects.get(
            id=initial_offer.id
        )  # Busca a mesma oferta pelo ID
        self.assertEqual(updated_offer.preco, decimal.Decimal("180.50"))
        self.assertGreater(
            updated_offer.data_captura, initial_offer.data_captura
        )  # Data de captura deve ser atualizada
        ...
        self.assertEqual(
            str(messages[0]),
            (
                f"Oferta para '{self.product1.nome}' na '{self.store1.nome}' "
                f"atualizada com sucesso para R$180.50!"
            ),
        )
    ...
    def test_manage_offers_delete_offer_success(self):
        ...
        offer_to_delete = Oferta.objects.create(
            produto=self.product2, loja=self.store2, preco=decimal.Decimal("50.00")
        )
        offer_count_before = Oferta.objects.count()
        ...
        self.assertEqual(
            str(messages[0]),
            (
                f"Oferta de '{self.product2.nome}' na '{self.store2.nome}' "
    ...
    def test_manage_offers_delete_non_existent_offer(self):
        ...
        self.assertEqual(
            Oferta.objects.count(), offer_count_before
    ...
    def test_manage_offers_add_invalid_data(self):
        ...
        self.assertEqual(
            Oferta.objects.count(), offer_count_before
        )  # Nenhuma oferta criada/atualizada

        self.assertContains(
            response, "Erro no campo &#x27;preco&#x27;: Digite um número."
        )
    ...
    def test_manage_offers_edit_prepopulate_form(self):
        ...
        existing_offer = Oferta.objects.create(
            produto=self.product1, loja=self.store1, preco=decimal.Decimal("123.45")
        )
        ...
        self.assertEqual(
            response.status_code, 200
        )  # Deve re-renderizar a página com o formulário populado
        ...
        self.assertContains(
            response, f'<option value="{self.product1.id}" selected="selected">'
        )
        self.assertContains(
            response, f'<option value="{self.store1.id}" selected="selected">'
        )
        ...
        self.assertEqual(
            str(messages[0]),
            (
                f"Editando oferta de '{existing_offer.produto.nome}' na "
                f"'{existing_offer.loja.nome}'."
            ),
        )
